...
static int pm_add_patch(patch_map* px_param_map, char* s_input_patch, int i_param_num_supress_dups);
...
static int pm_add_patch(patch_map* px_param_map, char* s_input_patch, int i_param_num_supress_dups)
{
  ...
  for (i_ypos = i_ypatch_pos; i_ypos < (i_ypatch_pos + i_ypatch_len); i_ypos++)
  {
    ...
    {
      px_param_map->aac_patches[i_ypos][i_xpos] += (1 - i_param_num_supress_dups);

      if (px_param_map->aac_patches[i_ypos][i_xpos] > 1)
      {
	// New dup
	i_patch_dups++;
...
int main(void)
{
  ...
  while (fgets(s_line_buf, sizeof(s_line_buf) , px_file) != NULL)
  {
    ...
    pm_add_patch(&x_patch_map, s_line_buf, 0);
  }
  // Now the whole canvas has been painted, so run it once more
  rewind(px_file);
  ...
  while (fgets(s_line_buf, sizeof(s_line_buf) , px_file) != NULL)
  {
    ...
    if (pm_add_patch(&x_patch_map, s_line_buf, 1) == 0)
    {
      printf("Id %s does not overlap\n", s_line_buf);
    }
  }
  fclose(px_file);
  pm_free(&x_patch_map);
